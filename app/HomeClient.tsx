...
export default function HomeClient({ events }: { events: FightEvent[] }) {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(async ({ data }) => {
        ...
        try {
          pending = localStorage.getItem("fightbase:pending_notif_pref");
        } catch (e) {
          // ignore
        }
        if (pending !== null) {
    ...
    if (error) {
      ...
      try {
        localStorage.setItem(
          "fightbase:pending_notif_pref",
          String(regEmailNotif)
        );
      } catch (e) {
        // ignore
      }
  ...
  const fighterList = useMemo(() => {
    const names = Array.from(
      new Set([...allFighterNames(events), ...Object.keys(fightersData)])
    ).sort();
    return names.map((name) => ({
      name,
      upcoming: upcomingFightsFor(events, name),
    }));
  }, [fightersData]);
  ...
  return (
    <div className="max-w-[480px] md:max-w-3xl lg:max-w-5xl mx-auto min-h-screen pb-10">
              ...
              <form onSubmit={handleAuthSubmit} className="flex flex-col gap-3">
                ...
                {authMode === "signup" && (
                  <label className="flex items-start gap-2.5 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={regEmailNotif}
                      onChange={(e) => setRegEmailNotif(e.target.checked)}
                      className="mt-0.5 accent-accent"
                    />
                    <span className="text-[13px] text-muted leading-snug">
                      {L.regEmailNotifLabel}
                    </span>
                  </label>
                )}
                <button
      ...
      <div className="px-5 pt-5 border-t border-border mt-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {promotionsWithPage(events).map((p) => (
            <Link
              key={p}
